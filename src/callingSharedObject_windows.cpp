#include <iostream>
#include <windows.h> // Windows의 DLL 관련 함수
...
{
    // 동적 라이브러리(.dll) 불러오기
    HMODULE handle = LoadLibrary("mylib_windows.dll"); // Windows용 DLL 경로 설정
    if (!handle)
    {
    ...
    GravityForces_t GravityForces = (GravityForces_t)GetProcAddress(handle, "GravityForces");
    if (!GravityForces)
    {
        std::cerr << "Failed to load GravityForces function: " << GetLastError() << std::endl;
        FreeLibrary(handle);
        return EXIT_FAILURE;
    }
    ...
    GravityForces_work_t GravityForces_work = (GravityForces_work_t)GetProcAddress(handle, "GravityForces_work");
